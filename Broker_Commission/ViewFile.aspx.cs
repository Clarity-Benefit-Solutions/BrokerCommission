...
namespace Broker_Commission
{
    public partial class ViewFile : System.Web.UI.Page
    {
        ...
        protected void LoadEditTable(int headerID)
        {

            string bid = Request.QueryString["BID"].ToString();
            DataTable datat = datatable(bid);
            List<STATEMENT_DETAILS_ADD> list = new List<STATEMENT_DETAILS_ADD>();
            list = (from DataRow dr in datat.Rows
                           select new STATEMENT_DETAILS_ADD()
                           {
        ...
        protected void DataLoad()
        {
            ...
            DataTable dataM = dataTable_master(bid);
        ...
        protected DataTable datatable(string bid)
        {
        ...
        protected DataTable dataTable_master(string bid)
        {
            DataTable dt = new DataTable();
        ...
        {
            lbl_literal.Text = stringb(dtp).ToString();
        }
        ...
        protected void btn_edit_OnClick(object sender, EventArgs e)
        {
            ...
            string bid = Request.QueryString["BID"].ToString();
            DataTable datat = datatable(bid);
            String htmlText = stringPDF(datat).ToString();
        ...
        {
            string bid = Request.QueryString["BID"].ToString();
            int statementID = int.Parse(bid);

            STATEMENT_DETAILS_ADD model = new STATEMENT_DETAILS_ADD()
            {
                HEADER_ID = Request.QueryString["HEADER_ID"] == null? 0 : int.Parse(Request.QueryString["HEADER_ID"].ToString()),
                BROKER_ID = getBrokerID(statementID),
                BROKER_NAME = getBrokerName(statementID),
                ...
                UNIT = "Per Qt",
                PAYLOCITY_ID = getPaylocityID(getBrokerID(statementID)),
        ...
        {
            int id = 0;
            var statementModel = db.STATEMENT_DETAILS.Where(x => x.HEADER_ID == statementID).FirstOrDefault();
